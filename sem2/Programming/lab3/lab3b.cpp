...
int partition (vector <Product>& products, int low, int high)
{
    ...
    {
        if (products[j].production_date.countDays() >= pivot.production_date.countDays()) 
        {
            i++;
...
void quickSort (vector <Product>& products, int low, int high) //production_date
{
    if (low < high)
    {
        int p = partition (products, low, high);
        quickSort (products, low, p - 1);
        quickSort (products, p + 1, high);
    }
}

int maxElement (int M, int quantity)
{
    if (M > quantity) return M; 
    else return quantity;
}

vector<Product> countSort(vector<Product>& inputArray)
{
    ...
    for (int i = 0; i < N; i++)
    {
        int intValue = round(inputArray[i].quantity);
        M = maxElement(M, intValue);
    }
    ...
    vector<Product> outputArray(N);
 
    for (int i = N - 1; i >= 0; i--)
    {
        int intValue = round(inputArray[i].quantity);
        outputArray[countArray[intValue] - 1] = inputArray[i];
...
void radixSort(vector<Product>& arr) 
{
    int max_days = 0;
    for (const Product& prod : arr) 
    {
        int days = prod.production_date.countDays() + prod.storage_period;
        if (days > max_days)
            max_days = days;
    }
    ...
    vector<vector<Product> > buckets(10);
    ...
    for (int i = 0; i < digits; i++) 
    {
        ...
        {
            int days = prod.production_date.countDays() + prod.storage_period;
            int digit = (days / exp) % 10;
            buckets[digit].push_back(prod);
        }

        arr.clear();
        for (vector<Product>& bucket : buckets) 
        {
    ...
    do 
    {
        ...
        switch(choice) 
        {
            ...
            case 6:
            {
                ...
                {
                    case 1:
                        quickSort(store.products, 0, store.products.size()-1);
                        break;
                    case 2:
                        store.products = countSort(store.products);
                        break;
                    case 3:
                        radixSort(store.products);
                        break;
                default:
                    cout << "Wrong type.\n";
                }
...
void benchmark()
{
    ...
    for(int i = 0; i <3; i++)
    {
        ...
        auto startCountSort = chrono::high_resolution_clock::now();
        store.products = countSort(store.products);
        auto endCountSort = chrono::high_resolution_clock::now();
        
        auto startRadixSort = chrono::high_resolution_clock::now();
        radixSort(store.products);
        auto endRadixSort = chrono::high_resolution_clock::now();
        ...
        auto durationQuickSort = chrono::duration_cast<chrono::milliseconds>(endQuickSort - startQuickSort);
        auto durationCountSort = chrono::duration_cast<chrono::milliseconds>(endCountSort - startCountSort);
        auto durationRadixSort = chrono::duration_cast<chrono::milliseconds>(endRadixSort - startRadixSort);
        ...
        cout << "Time quick sort: " << durationQuickSort.count() << endl;
        cout << "Time counting sort: " << durationCountSort.count() << endl;
        cout << "Time radix sort: " << durationRadixSort.count() << endl;
