...
## Тестування | Обробка помилок

Система реалізує стійкість до збоїв (Fault Tolerance). Нижче наведено таблицю можливих помилок і способів їх спровокувати для перевірки:

| Error Case | Поведінка Менеджера | Trigger |
| :--- | :--- | :--- |
| **Artificial FAIL** (Worker Crash) | Воркер імітує критичний збій (`System.exit`). Менеджер фіксує відсутність відповіді, чекає на timeout та повідомляє про помилку, не припиняючи власну роботу. | Ввести число `2` як аргумент $x$. |
| **Timeout** (Зависання) | Якщо обчислення тривають довше **10 секунд**, Менеджер автоматично перериває операцію, щоб уникнути блокування ресурсів. | Спрацьовує автоматично при `Artificial FAIL` (оскільки воркер вмирає і не відповідає). |
| **User Interrupt** | Користувач примусово зупиняє роботу. Менеджер виконує Graceful Shutdown (закриває сокети, вбиває процеси). | Натиснути `q` та `Enter` під час обчислень. |
